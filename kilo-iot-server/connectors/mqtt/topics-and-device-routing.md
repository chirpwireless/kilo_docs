...
MQTT device routing associates an incoming message with the correct registered device and reading in Kilo IoT Server. A **topic** is the message's named channel. The routing pattern identifies the device from that channel; field mapping identifies the measurements in its message content, or **payload**.
