...
- **Connectors** — Establish protocol-level connections between your infrastructure and the server. Current connector types: **LNS** (LoRaWAN) and **Tracker** (vehicle trackers — OBD2, CAN bus, standalone GPS). MQTT is in preview. Opening an LNS connector shows **LoRaWAN Gateways** and **Connected Devices** tabs. Each connector shows its name, last data received, and connected device count. Devices are registered through the device management dialog.
