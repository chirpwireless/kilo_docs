...
6. Click **Conditions: N** to open the Conditions modal. Set a **Default color** — the color the reading falls back to whenever none of your conditions match the current value — then for each band click **Add condition** and fill the row — enter a **Condition name**, set **Data type** to **Number** (the condition's own Data type, not the metric row's), because the reservoir holds 8,000 liters, enter **From** 0 (an empty reservoir) and **To** 8000 (a full one), and pick a **Color**. Then you can enter the color levels. For example:
...
10. Set **Min value** to **0** (an empty reservoir) and **Max value** to **8000** (its full 8,000-liter capacity) — the same range the conditions use. The wedge fills the whole circle at 8,000 liters.
11. Switch on **Display data legend** if useful, then click **Save**.

The result: a solid disc that reads as a strong block of color from across the room — full and green while the reservoir is well stocked, shrinking through yellow to red as it is drawn down. The same steps fit any capacity or proportion reading; change the device, the **Min value**/**Max value**, and the bands to match.
