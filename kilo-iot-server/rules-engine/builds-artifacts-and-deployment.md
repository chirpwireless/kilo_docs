...
Saving a rule preserves your work. Building a rule validates it and produces a deployable artifact. Deploying an artifact starts the rule processing its selected sensor or trigger source. These are deliberate, separate steps — the platform never runs automation logic that has not been explicitly built and deployed.
...
|---|---|---|
| **Running** | Green | The artifact is deployed and waiting for its selected sensor or trigger source |
...
Stopping is a deliberate pause action — use it when you want a rule to stop responding to its selected source without deploying a different version.
...
The status changes to **Stopped** (orange). The rule stops accepting new starts immediately. Sensors continue reporting and triggers continue monitoring, but the stopped rule does not run from either source.
...
- The rule stops responding to new sensor readings or trigger activations.
